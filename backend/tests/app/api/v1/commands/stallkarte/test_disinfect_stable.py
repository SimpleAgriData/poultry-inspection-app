...
from app.api.v1.commands.stallkarte.disinfect_stable import (
    RequestBody,
    handler,
)
...
class TestDisinfectStable:
    ...
    def test_disinfect_stable(
        ...
        assert (
            response.message
            == "Stable disinfection completed successfully for cycle 'fattening'"
        )
        ...
        assert checklist.stable_disinfected.date == today
        assert checklist.stable_disinfected.disinfectant == "Standard Detergent"
        assert checklist.stable_disinfected.dosis == "2 liters per 1000 square meters"
