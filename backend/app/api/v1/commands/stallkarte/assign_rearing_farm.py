...
class RequestBody(BaseModel):
    ...
    farm_id: int = Field(description="ID of the rearing farm")


@router.post(
    "/assign-rearing-farm",
    response_model=MessageResponse,
    name="Assign Rearing Farm",
...
def handler(
    ...
    logger.info(f"Assigned rearing farm '{farm.id}' to Stallkarte ID '{stallkarte.id}'")

    return MessageResponse(message=f"Rearing farm '{farm.id}' assigned successfully")
