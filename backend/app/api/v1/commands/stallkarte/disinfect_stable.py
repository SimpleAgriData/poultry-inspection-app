...
class RequestBody(BaseModel):
    ...
    date: datetime.date | None = Field(description="Date of stable disinfection")
...
def handler(
    ...
    logger.info(
        f"Recorded stable disinfection for Stallkarte ID '{stallkarte.id}' in cycle "
        f"'{request_body.cycle}' with disinfectant '{request_body.disinfectant}' and "
        f"dosis '{request_body.dosis}' on date '{request_body.date}'"
    )

    return MessageResponse(
        message=f"Stable disinfection completed successfully for "
